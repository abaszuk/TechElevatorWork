...
document.addEventListener("DOMContentLoaded", () => {
    ...
    inputDescription.addEventListener("blur", () => {
      exitDescriptionEdit(inputDescription, false);
    });

    inputDescription.addEventListener('keyup', (event) => {
      if(event.key === "Enter"){
        exitDescriptionEdit(inputDescription, true);
      }
    });
...
function saveReview() {
  ...
  //First get all the input elements we want to 
  ...
  //Create a new review matching the same field names
  ...
  //add our new review to our array of existing reviews
  reviews.push(newReview);

  //and now make sure that we add the review to the page 
  //so that we can see it
  displayReview(newReview);

  //clear the form values and hide the form
  resetFormValues();
  showHideForm();
}
